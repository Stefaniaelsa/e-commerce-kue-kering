...
<html lang="id">
  ...
  <body class="bg-[#fff7f4] text-[#4e3d3a] font-sans">
    ...
    <section class="relative h-72 md:h-96 overflow-hidden">
        <div id="slider" class="absolute inset-0 bg-center bg-cover transition-all duration-1000"></div>
        <div class="absolute inset-0 bg-pink-900 bg-opacity-40 flex flex-col items-center justify-center text-white text-center px-4">
    ...
    <script>
        ...
        setInterval(changeBackground, 4000); // Ganti tiap 4 detik
    </script>
...
<section class="py-8 px-6">
  ...
  <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
    <div class="bg-white shadow rounded p-4 text-center hover:shadow-lg transition">
      <a href="#best-seller" class="bg-white shadow rounded p-4 text-center hover:shadow-lg transition block">
    ...
    <div class="bg-white shadow rounded p-4 text-center hover:shadow-lg transition">
      <a href="#favorit" class="bg-white shadow rounded p-4 text-center hover:shadow-lg transition block">
        <i class="fas fa-heart text-3xl text-pink-400 mb-2"></i>
        <p class="font-semibold">Favorit</p>
      </a>

    </div>
    <a href="{{ url('/produk') }}" class="bg-white shadow rounded p-4 text-center hover:shadow-lg transition block">
...
<!-- Best Seller Section -->
<section id="best-seller" class="mb-8">
  <h2 class="text-xl font-semibold mb-4">Best Seller</h2>
  <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
    @forelse ($bestSellerProduk as $produk)
      <div class="bg-white rounded-lg shadow hover:shadow-lg transition p-4">
      <img src="{{ asset('images/' . $produk->gambar) }}" alt="{{ $produk->nama }}" class="w-full h-40 object-cover rounded mb-2" />
